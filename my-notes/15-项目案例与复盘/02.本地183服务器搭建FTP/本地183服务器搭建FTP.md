...
有一台Linux服务器  搭载 Ubuntu 22.04.5 LTS
...
```shell
sudo useradd -m -s /bin/bash HtFtpUser

cat /etc/passwd | grep HtFtpUser
```
...
sudo chown -R HtFtpUser:HtFtpUser /ftp/

# 给用户写权限
sudo chmod -R u=rwx /ftp/
...
# 
sudo vim /etc/vsftpd/vu/HtFtpUser

# ftpmusic文件内容如下：
guest_username=HtFtpUser
