...
- 专业解释（了解一下）：
  ...
  这种动态获取信息及动态调用对象方法的功能称为Java语言的反射机制。
...
Class clazz1 = Class.forName("com.test.reflectdemo.Student");
...
- .class(字节码文件)： .java文件编译后生成的class文件
...
如果当前获取到的是私有的，必须要临时修改访问权限（.isAccess(true)），否则无法使用
...
​	在这个故事中，经纪人作为代理，如果别人让邀请大明星去唱歌，打篮球，经纪人就增强功能；

​	但如果别人让大明星去扫厕所，经纪人就要拦截，不会去调用大明星的方法。
